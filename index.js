...
function buildPanel() {
  ...
  const settingsBtn = h('button', { text: '⚙️', title: '设置', onclick: () => toggleSettings() });
...
async function loadSettings() {
  try {
    ...
    if (bodyEl) { bodyEl.innerHTML = ''; bodyEl.appendChild(h('div', { class: 'la-dim', text: '读取配置失败: ' + e.message })); }
    return;
  }
  renderSettings();
}
...
function renderSettings() {
  ...
  const saveBtn = h('button', { text: '保存', onclick: () => saveSettings() });
  const backBtn = h('button', { text: '返回', onclick: () => toggleSettings() });
...
async function saveSettings() {
  try {
    ...
    bodyEl.insertBefore(tip, bodyEl.firstChild);
  } catch (e) {
